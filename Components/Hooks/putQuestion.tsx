...
const usePUT_Question = (
  ...
  const triggerPost = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_BASE_URL}/api/admin/questions${QId}`, {
    ...
    if (response.status === 204) {
        ...
        setData({ success: true, message: 'Вопрос успешно изменен' }); // Set a success state indicator
  ...
  }, [question, QId]); // Dependencies: Recreate triggerPost if question or QType changes
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.success) {
         alert(`Вопрос No: ${QId} был успешно изменен`);
         ...
         alert(`Ошибка при изменении вопроса No: ${QId}: ${data.message || 'Сервер сообщил об ошибке'}`);
      }
    } else if (error) {
       // Handle network or other errors caught in the catch block
       alert(`Ошибка при изменении вопроса No: ${QId}: ${error}`);
    }
  }, [data, error, QId]); // Dependencies: Run this effect when data, error, or QType changes
...
export default usePUT_Question;